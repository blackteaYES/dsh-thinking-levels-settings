window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		function namespaceView(response) {
			if (!response || response.result.ok !== true) throw new Error(response.result.ok === false ? response.result.error.message : "读取设置失败");
		...
		function pageState(view) {
			...
			for (const id of Object.keys(configuredProviders)) {
				...
				const models = profile.models.map((entry) => modelView(entry)).filter((entry) => entry !== null);
		...
		function serializedEfforts(draft) {
		...
		function ThinkingLevelsSection({ api }) {
		...
		function ModelEditor({ api, state, provider, model, onCommitted }) {
			...
			const save = async (nextDraft) => {
				...
				try {
					...
					if (!response.result.ok) {
						if (response.result.error.code === "settings-conflict") throw new Error("设置已在其他位置更新，请刷新后重试");
						throw new Error(response.result.error.message);
					}
					onCommitted(pageState(Object.assign({}, response.result.value, { writable: state.writable })));
					setNotice({
						type: "success",
			...
			const applyPreset = (efforts) => {
				const next = {};
				for (const [id] of LEVELS) if (Object.prototype.hasOwnProperty.call(efforts, id)) next[id] = efforts[id];
				setDraft(next);
				save(next);
			};
			return react.createElement("article", { className: "tl-model" }, react.createElement("header", { className: "tl-model-header" }, react.createElement("div", null, react.createElement("div", { className: "tl-model-name" }, model.name), model.name !== model.id ? react.createElement("div", { className: "tl-model-id" }, model.id) : null, react.createElement("div", { className: "tl-current" }, `当前配置：${summaryOf(model.reasoningEfforts)}`)), react.createElement("div", { className: "tl-presets" }, ...PRESETS.map(([label, efforts]) => react.createElement("button", {
				key: label,
				type: "button",
			...
			}, "关闭思考"))), react.createElement("div", { className: "tl-levels" }, ...LEVELS.map(([id, label]) => {
				...
				return react.createElement("label", {
					...
					disabled: busy || !state.writable,
					onChange: () => toggle(id)
				}), react.createElement("span", { className: "tl-label" }, label), checked ? react.createElement("input", {
					className: "tl-wire",
